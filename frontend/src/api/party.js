...
// 파티 정보 조회
async function requestGetPartyInfo (clubId,partyId,success,fail) {
...
// 파티원 목록 조회
async function requestGetPartyMemberList (clubId,partyId,success,fail) {
    local.get(`clubs/${clubId}/${API_URL}/${partyId}/participants`).then(success).catch(fail);
}

// 파티원 생성
async function requestPostPartyMember (clubId,partyId,success,fail) {
    local.post(`clubs/${clubId}/${API_URL}/${partyId}/participants`).then(success).catch(fail);
}

// 파티원 조회
async function requestGetPartyMember (clubId,partyId,participantId,success,fail) {
